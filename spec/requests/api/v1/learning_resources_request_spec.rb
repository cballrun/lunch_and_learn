    ...
    describe 'happy path' do
      it 'sends one video and multiple images for the request' do
        VCR.use_cassette('laos_data') do
          ...
          expect(learning_resource[:attributes]).to_not have_key(:kind)
          expect(learning_resource[:attributes]).to_not have_key(:videoId)
          expect(learning_resource[:attributes]).to_not have_key(:length)
          expect(learning_resource[:attributes]).to_not have_key(:color)
          ...
          expect(learning_resource[:attributes][:video]).to_not have_key(:description)
          expect(learning_resource[:attributes][:video]).to_not have_key(:etag)
          
          expect(learning_resource[:attributes][:images]).to be_a(Array)
          expect(learning_resource[:attributes][:images].first).to be_a(Hash)
          expect(learning_resource[:attributes][:images].first.count).to eq(2)
          ...
          expect(learning_resource[:attributes][:images].first).to_not have_key(:links)
          expect(learning_resource[:attributes][:images].first).to_not have_key(:blur_hash)
        end
      end
    end
